...
from .utils import loadmat, validate_data_program
import itertools
...
class TrialsTable(TimeIntervals):
    ...
    __columns__ = (
        {"name": "states", "description": "The states", "index": True, "table": True, "required": False},
        {"name": "events", "description": "The events.", "index": True, "table": True, "required": False},
        {"name": "actions", "description": "The actions.", "index": True, "table": True, "required": False},
    )
    ...
    def populate_from_matlab(self, **kwargs):
        ...
        for trial in states_data:
            ...
            if not isinstance(trial_actions, list):
        ...
        for i in range(len(start_times)):
            self.add_trial(
                ...
                states=states_index_ranges[i],
                events=events_index_ranges[i],
                actions=action_index_ranges[i],
            )
...
class StatesTable(TimeIntervals):
    ...
    __columns__ = (
        {"name": "state_type", "description": "The state type", "table": True, "required": True},
        {"name": "start_time", "description": "The state start time", "required": True},
        {"name": "stop_time", "description": "The state stop time", "required": True},
    )
    ...
    @docval(
        *get_docval(DynamicTable.__init__, "id", "columns", "colnames"),
        ...
        {"name": "state_types_table", "type": "StateTypesTable", "doc": "The states table.", "default": None},
        allow_positional=AllowPositional.ERROR,
    )
    def __init__(self, **kwargs):
        kwargs["name"] = "states"
    ...
    def _generate_state_end_times(self, **kwargs):
        # [i['TrialPath'] for i in data['States']]
        # *Note*: matlab_file=loadmat(beadl_path) ---> data=matlab_file['BeadlData']
    ...
    def populate_from_matlab(self, **kwargs):
        ...
        validate_bool, unique_keys = (
            self._all_states_validate(all_states=all_states)[0],
            self._all_states_validate(all_states=all_states)[1],
        )

        if validate_bool:
            # validate state_types from matlab file with task program xml
            state_types_table_data = state_types_table["state_name"].data
            valid = validate_data_program(data=unique_keys, program=state_types_table_data)
            if valid:
                ...
                region = state_types_table.create_region(
                    name="type", region=state_idx_list, description="idx to the names of states"
                )
...
class StateTypesTable(DynamicTable):
    __columns__ = ({"name": "state_name", "description": "The name of the state type", "required": True},)
    ...
    @docval(
        *get_docval(DynamicTable.__init__, "id", "columns", "colnames"),
        ...
        {"name": "beadl_task_program", "type": BEADLTaskProgram, "doc": "the BEADLTaskProgram used", "default": None},
        {
    ...
    def __init__(self, **kwargs):
        ...
        if self.beadl_task_program is None:
...
class EventTypesTable(DynamicTable):
    __columns__ = ({"name": "event_name", "description": "The name of the event type", "required": True},)
...
class ActionTypesTable(DynamicTable):
    __columns__ = ({"name": "action_name", "description": "The name of the action type", "required": True},)
...
class ActionsTable(DynamicTable):
    __columns__ = (
        {"name": "timestamp", "description": "The action time", "required": True},
        {"name": "duration", "description": "Duration of the action in seconds.", "required": False},
        {"name": "action_type", "description": "The action type", "table": True, "required": True},
        {"name": "value", "description": "The action value", "required": True},
    )
    ...
    @docval(
        *get_docval(DynamicTable.__init__, "id", "columns", "colnames"),
        ...
        {"name": "action_types_table", "type": "ActionTypesTable", "doc": "The ActionTypesTable.", "default": None},
        allow_positional=AllowPositional.ERROR,
    )
    def __init__(self, **kwargs):
        kwargs["name"] = "actions"
    ...
    @docval(
        ...
        {"name": "duration", "type": float, "doc": "Duration of the action in seconds.", "default": None},
        {
            "name": "value",
            "type": str,
    ...
    def populate_from_matlab(self, **kwargs):
        ...
        valid = validate_data_program(data=unique_action_names, program=action_types_table_data)
        ...
        if valid:
            ...
            # create a dynamic table region where the data is the idx
            region = action_types_table.create_region(
                name="action_type", region=action_idx_list, description="idx to the names of actions"
            )
