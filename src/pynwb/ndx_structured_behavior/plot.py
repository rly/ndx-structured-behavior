...
def show_by_type_and_value(
    table: Union[ActionsTable, EventsTable, pd.DataFrame],
...
def plot_events(
    events: Union[EventsTable, pd.DataFrame],
    ...
    if keep_yticks:
        ...
        plt.yticks(
            np.arange(y_offset, y_offset + len(y_tick_labels)).tolist() + tp, y_tick_labels + tl, fontsize=fontsize
        )
    else:
...
def plot_actions(
    actions: Union[ActionsTable, pd.DataFrame],
...
def plot_states(
    states: Union[StatesTable, pd.DataFrame],
    ...
    patches = [
        mpl.patches.Rectangle((x_start[i], y_values[i]), x_range[i], y_height, color=rectangle_color)
    ...
    if keep_yticks:
        ...
        tl = [t.get_text() for t in plt.yticks()[1]]
        plt.yticks(
            np.arange(y_offset, y_offset + len(y_tick_labels)).tolist() + tp,
            np.concatenate([y_tick_labels, tl]),
            fontsize=fontsize,
        )
    else:
        plt.yticks(np.arange(y_offset, y_offset + len(y_tick_labels)).tolist(), y_tick_labels, fontsize=fontsize)
        plt.ylabel(y_label, fontsize=fontsize)
    plt.ylabel(y_label, fontsize=fontsize)
    return fig
...
def plot_trials(
    trials: Union[TrialsTable, pd.DataFrame],
    states: StatesTable,
    state_types: StateTypesTable,
    ...
    patches = [mpl.patches.Rectangle((x_start[i], y_values[i]), x_range[i], y_height) for i in range(len(x_start))]
    ax = plt.gca()
    ax.add_collection(
    ...
    for i, r in enumerate(patches):
        ...
        ax.annotate(
            trials_df.index[i], (cx, cy), color="w", fontsize=fontsize - 3, ha="center", va="center"  # weight='bold',
        )
    plt.xticks(x_start)
...
def compute_state_transition_matrix(states: Union[StatesTable, pd.DataFrame], state_types: StateTypesTable):
    """
    ...
    state_transition_probability_df = state_transition_count_df.divide(
        state_transition_count_df.sum(axis="columns"), axis="rows"
    )
...
def plot_state_transition_graph(
    ...
    networkx is required to run this function.
    ...
    try:
        ...
        raise ImportError("Please install the networkx package to use this function. E.g., via 'pip install networkx'")
